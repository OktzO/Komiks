...
const VALID = ['komiku', 'bacakomik', 'thrive', 'shinigami', 'manhwaindo', 'webtoon'] as const;
...
export default function SourceMonitorPage({ source }: { source: string }) {
  ...
  if (!(VALID as readonly string[]).includes(source)) {
    return (
      <main className="max-w-3xl mx-auto px-4 pt-4 pb-16">
        <a href="/status" className="text-sm text-secondary hover:text-primary">← Kembali ke status</a>
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 pt-4 pb-16 anim-slide-up">
      <a href="/status" className="inline-flex items-center gap-2 text-sm text-secondary hover:text-primary transition-colors mb-6">
