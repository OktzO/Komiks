...
export function SourceSwitcher({
  ...
  useEffect(() => {
    if (!navigating) setPendingSource(null);
  }, [navigating]);
  ...
  // plus label "Memuat…" di chip yang dipilih. Tanpa useTransition (Next-only),
  // pakai state navigating yang di-clear oleh pageshow saat navigasi selesai.
  const SwitchingBar = navigating ? <div className="route-progress" aria-hidden="true" /> : null;
  ...
  const onPick = (source: string) => {
    ...
    const go = () => {
      if (mode === 'detail') {
        window.location.href = typedUrl(safeType(type), canonicalSlug ?? link.sourceSlug);
      } else {
            ...
            {ordered.map((s) => {
              ...
              return (
                <button
                  key={s}
                  onClick={() => onPick(s)}
                  disabled={!link?.hasChapterList || navigating}
