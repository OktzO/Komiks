...
// nilai dari dashboard Cloudflare Pages. Fallback localhost = dev only.
const envPublic = (key: string): string => {
  let v = '';
  try {
    v = (import.meta.env as Record<string, string | undefined>)[key] ?? '';
  } catch {}
  if (!v && typeof process !== 'undefined' && process.env) {
    v = process.env[key] ?? process.env[key.replace('PUBLIC_', '')] ?? '';
  }
  return v;
};
