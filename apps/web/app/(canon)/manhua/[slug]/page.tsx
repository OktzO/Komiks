...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }>; }): Promise<Metadata> {
  ...
  try {
    ...
    if (resolved.type !== 'manhua') permanentRedirect(`/${resolved.type}/${encodeURIComponent(slug)}`);
    const s = await detailPromise;
    return { title: s.title, description: (s.synopsis ?? '').replace(/\s+/g, ' ').trim().slice(0, 160) || undefined };
  } catch { notFound(); }
}

export default async function ManhuaDetail({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  return <CanonicalDetail type="manhua" slug={slug} />;
}
